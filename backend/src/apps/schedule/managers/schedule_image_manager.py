...
class ScheduleImageManager:
    ...
    async def update(
        ...
        async with self.db.db_session() as session:
            ...
            if not data and not is_local and not filename:
            ...
            async with maybe_lock(lock, use_lock=locking):
                ...
                try:
                    ...
                    if image:
                        ...
                        target = f"local/{filename}" if filename else None
                        if target is None and schedule_image.is_local is False:
                            raise HTTPException(
                                400, "Недостаточно данных для редактирования"
                            )
                        data["is_local"] = True
                        if target:
                            file_data = self.storage.read_file(
                                path=filename, is_local=True
                            )
                            if file_data is None:
                                raise HTTPException(400, detail="Ошибка чтения файла")
                            ...
                            if new_image is None:
                                raise HTTPException(
                                    400, detail="Ошибка сохранения файла"
                                )
                            meta = self.storage.read_file_metadata(new_image)
                            if meta is None:
                                raise HTTPException(400, detail="Файл не найден")
                            ...
                            if meta_local and not self._same_content(
                                meta_local, meta_static
                            ):
                                ...
                                subdir, _, filename = old_image.rpartition("/")
                                new_image = self.storage.save(
                    ...
                    if is_local and old_image and new_image:
                        if new_image == old_image:
                            self.storage.restore_file(old_image)
                        else:
                            self.storage.delete(new_image)
                    raise
