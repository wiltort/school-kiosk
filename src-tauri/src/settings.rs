//! Настройки приложения, хранящиеся в файле на диске.
//!
//! Настройки записываются в JSON-файл и могут быть изменены:
//!   - при установке (инсталлятор пишет `settings.json`);
//!   - из админ-панели (команды `get_settings` / `set_static_dir`);
//!   - вручную (файл легко отредактировать).
//!
//! Файл по умолчанию лежит в каталоге конфигурации приложения Tauri
//! (`%APPDATA%\com.schoolkiosk.app\settings.json` на Windows). Путь можно
//! переопределить переменной окружения `SCHOOL_KIOSK_SETTINGS_FILE`
